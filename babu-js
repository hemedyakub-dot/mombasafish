...
(function () {
  ...
  var KB = [
    ...
    { k: ['open', 'hours', 'where are you', 'shop', 'location', 'visit'], a: 'The shop is in Mombasa — there is a pin on the map at the bottom of every page.\n\nFor hours and whether the boats have come in today, ask the people at the shop. They will know; I only know fish.', f: [], wa: true }
  ];
  ...
  function match(qRaw) {
    ...
    KB.forEach(function (e) {
      var s = 0;
      e.k.forEach(function (kw) { if (q.indexOf(' ' + kw) > -1) s += kw.length; });
  ...
  function answer(q) {
    ...
    setTimeout(function () {
      ...
      var cards = (m.f || []).map(cardHTML).join('');
      if (m.wa) cards += '<a class="bs-wa" href="' + waLink(q) + '">Ask the team on WhatsApp →</a>';
      say(m.a, 'b', cards);
    }, 260);
  }
  ...
  function open() {
    ...
    if (!opened) { opened = true; say(GREET, 'b'); }
