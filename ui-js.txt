...
(function () {
  ...
  var css =
    '.mfwa{position:fixed;z-index:129;right:18px;bottom:18px;width:56px;height:56px;border-radius:50%;'
    ...
    + '.mfwl{position:fixed;z-index:129;right:82px;bottom:34px;background:#032A2E;color:#F2EDE3;'
...
/* ============================================================
   ...
   Captured once per browser session and kept in sessionStorage,
   so it survives moving around the site but not a new visit.
   No cookies, no third party, nothing personal — it records the
   referring site and any campaign tag, never the person.
   Read by the order form so an order can say how it arrived.
   ============================================================ */
(function () {
  try {
    if (sessionStorage.getItem('mf_src')) return;   // first page of the visit only
    var q = new URLSearchParams(location.search);
    var bits = [];

    var src = q.get('utm_source') || q.get('source');
    var med = q.get('utm_medium');
    var cmp = q.get('utm_campaign') || q.get('cmp');
    var fb  = q.get('fbclid');

    if (src) bits.push(src + (med ? '/' + med : ''));
    else if (fb) bits.push('meta');
    else if (document.referrer) {
      var h = '';
      try { h = new URL(document.referrer).hostname.replace(/^www\./, ''); } catch (e) {}
      if (h && h.indexOf(location.hostname) === -1) {
        if (/facebook|fb\.|instagram|messenger/.test(h)) bits.push('meta');
        else if (/google|bing|duckduckgo|yahoo/.test(h)) bits.push('search');
        else if (/whatsapp|wa\.me/.test(h))              bits.push('whatsapp');
        else if (/tiktok/.test(h))                        bits.push('tiktok');
        else bits.push(h);
      }
    }
    if (!bits.length) bits.push('direct');
    if (cmp) bits.push('cmp:' + cmp);

    sessionStorage.setItem('mf_src', bits.join(' ').slice(0, 80));
    sessionStorage.setItem('mf_land', location.pathname.slice(0, 60));
  } catch (e) { /* private mode — no attribution, no harm */ }
})();
